    ...
    void buildMesh() {
        ...
        for (int x=0; x<CHUNK_SIZE_X; ++x) for (int y=0; y<CHUNK_SIZE_Y; ++y) for (int z=0; z<CHUNK_SIZE_Z; ++z) {
            ...
            auto shouldRenderFace = [&](int neighbor) {
                ...
                if (currentAlpha) {
                    // У alpha-блоков внутренние грани между alpha-соседями не рисуем,
                    // но грань у твёрдого блока остаётся видимой через прозрачные texel.
